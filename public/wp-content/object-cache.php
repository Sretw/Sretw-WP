 ...
 * Description: A persistent object cache backend powered by Redis. Supports Predis, PhpRedis, Relay, replication, sentinels, clustering.
 * Version: 3.0.3
 * Author: Namecheap
 * Author URI: https://www.namecheap.com/support/knowledgebase/article.aspx/10015/2279/easywp-plugins-cache-plugin-seo-plugin-and-blocked-plugins/
 * License: GPLv3
...
class WP_Object_Cache
{
  ...
  public function delete_group($group)
  {
    ...
    $multisite_safe_group =
      $this->multisite && !isset($this->global_groups[$group]) ? $this->blog_prefix . $group : $group;
    $redis_safe_group = $this->_key("", $group);
    if ($this->_should_persist($group)) {
  ...
  public function build_client_parameters($redis_server)
  {
    ...
    if (empty($redis_server)) {
      // Attempt to automatically load Redis config from the env.
  ...
  public function __construct()
  {
    ...
    if ($this->is_redis_failback_flush_enabled() && !empty($wpdb)) {
      ...
      $this->do_redis_failback_flush = (bool) $wpdb->get_results(
        "SELECT {$col2} FROM {$table} WHERE {$col1}='wp_redis_do_redis_failback_flush'"
      );
    ...
    $this->global_prefix =
      $this->multisite || (defined("CUSTOM_USER_TABLE") && defined("CUSTOM_USER_META_TABLE")) ? "" : $table_prefix;
